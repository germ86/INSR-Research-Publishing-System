...
- `document/build-profile`: development, review, production. Aliases `dev`, `prod`, `productive`, and `release` are normalized; the build profile also controls placeholder policy (`compact`, `show`, or `error`).
