...
class ReleaseMetadataStaticTests(unittest.TestCase):
    def test_reference_publication_entrypoints_exist_and_are_neutral(self):
        ...
        for name in ["paper.tex", "slides.tex", "handout.tex", "poster.tex", "content/reference-body.tex", "references.bib"]:
    ...
    def test_metadata_exports_are_generated_without_pending_doi(self):
        with tempfile.TemporaryDirectory() as tmp:
            proc = subprocess.run(["python3", "tools/insr_metadata.py", "export-all", "--outdir", tmp], cwd=ROOT, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def test_release_prepare_creates_manifest(self):
        proc = subprocess.run(["python3", "tools/insr_release.py", "prepare", "--version", "test-alpha"], cwd=ROOT, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.assertEqual(proc.returncode, 0, proc.stderr)
